...
export function computeReminders({ trainings, periodStart, players, myName, goals, isTrainer, auditLog }) {
  const list = [];
  ...
  if (isTrainer) {
    const recent = auditLog.filter((l) => l.action === "gewijzigd" && daysBetweenSafe(l.at.slice(0, 10)) <= 7);
    if (recent.length > 0) list.push(`${recent.length} eerdere invoer(en) deze week gewijzigd — bekijk het logboek bij Beheer.`);
  }
  return list;
}
