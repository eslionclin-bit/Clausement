...
export function doelGeschiedenisVoorSpeler(trainings, playerId, goalHistoryVoorSpeler, huidigDoel, exercises) {
  ...
  return gesorteerd.map((assignment, i) => {
    ...
    const aantal = trainings.filter(
      (t) =>
        t.date >= assignment.chosenAt &&
        (!eindDatum || t.date < eindDatum) &&
        t.spelers?.[playerId]?.doelRaw !== "" &&
        t.spelers?.[playerId]?.doelRaw !== undefined
    ).length;
...
// scoredetail op Standen.
export function doelRegelsVoorPeriode(trainings, playerId, periodStart, periodEnd, goalHistoryVoorSpeler, huidigDoel, exercises) {
  const toewijzingen = [...(huidigDoel ? [...goalHistoryVoorSpeler, huidigDoel] : goalHistoryVoorSpeler)].sort((a, b) =>
    a.chosenAt < b.chosenAt ? -1 : 1
  );

  function toewijzingOpDatum(datum) {
    let gevonden = null;
    for (const a of toewijzingen) {
      if (a.chosenAt <= datum) gevonden = a;
      else break;
    }
    return gevonden;
  }
  ...
  return trainings
    ...
    .map((t) => {
      const toewijzing = toewijzingOpDatum(t.date);
      const ex = toewijzing ? exercises.find((e) => e.id === toewijzing.exerciseId) : null;
      return {
        date: t.date,
        doelRaw: t.spelers[playerId].doelRaw,
        doel: t.spelers[playerId].doel,
        exerciseName: toewijzing?.exerciseName,
        metric: ex?.metric,
      };
    });
}
